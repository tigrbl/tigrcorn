# Delivery notes — Apache 2.0 migration, root Markdown reorganization, and Release certification wrapper fix
...
- the Release certification release-checkpoint regeneration path was hardened so rerunning `tools/create_release_assembly_checkpoint.py` and the Step 9 promotion overlay does not fail on brittle Markdown section replacement logic
...
- `PYTHONPATH=src python tools/create_release_assembly_checkpoint.py`
...
- the Release certification certification/documentation regression targeted by this maintenance update is fixed
