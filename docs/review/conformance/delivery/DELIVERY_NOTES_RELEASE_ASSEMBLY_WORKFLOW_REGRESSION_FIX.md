# Delivery notes — Release assembly workflow regression fix

This checkpoint fixes the two CI regressions reported against the promoted `0.3.8` package.

Fixed regressions:

- rerunning `tools/create_release_assembly_checkpoint.py` on an already promoted repository no longer rolls `docs/review/conformance/release_assembly.current.json` back to a pre-promotion state
- the preserved certification-environment bundle and `docs/review/conformance/certification_environment_freeze.current.json` are now resynchronized during the Release assembly assembly refresh path, so the status document no longer drifts from the release-root bundle after workflow regeneration
