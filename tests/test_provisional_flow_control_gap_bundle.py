...
class ProvisionalFlowControlGapBundleTests(unittest.TestCase):
    def test_bundle_index_registers_relative_non_certifying_bundle(self) -> None:
        ...
        self.assertIn('flow-control gap bundle', '\n'.join(payload['notes']))

    def test_mapping_file_covers_all_flow_control_review_scenarios(self) -> None:
        mapping_payload = json.loads((PROVISIONAL_ROOT / 'scenario_mapping.json').read_text(encoding='utf-8'))
        mappings = {entry['provisional_id']: entry['source_same_stack_id'] for entry in mapping_payload['mappings']}
    ...
    def test_each_provisional_result_is_explicitly_ineligible_for_release_gates(self) -> None:
        for scenario_id, source_id in EXPECTED_SCENARIOS.items():
            ...
            vectors = json.loads((PROVISIONAL_ROOT / scenario_id / 'source_local_vectors.json').read_text(encoding='utf-8'))
            self.assertTrue(result['passed'])
            self.assertTrue(result['provisional_non_certifying_substitution'])
            self.assertTrue(result['flow_control_review_only'])
            self.assertFalse(result['release_gate_eligible'])
            self.assertEqual(result['source_same_stack_scenario'], source_id)
            self.assertEqual(metadata['source_same_stack_scenario'], source_id)
