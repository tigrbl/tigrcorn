...
def test_control_frames_must_not_fragment():
    with pytest.raises(ProtocolError):
        parse_frame_bytes(encode_frame(OP_PING, b"a", fin=False))

def test_control_frames_must_be_small():
    with pytest.raises(ProtocolError):
        parse_frame_bytes(encode_frame(OP_PING, b"a" * 126, fin=True))

def test_invalid_reserved_opcode_rejected():
    with pytest.raises(ProtocolError):
...
def test_invalid_close_reason_utf8_rejected():
    payload = (1000).to_bytes(2, "big") + b"\xff"
    with pytest.raises(ProtocolError):
        decode_close_payload(payload)

async def test_accept_subprotocol_must_be_offered():
    writer = _FakeWriter()
    sender = _WSAppSend(
        writer=writer,
        server_header=None,
