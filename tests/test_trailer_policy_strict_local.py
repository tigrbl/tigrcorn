...
from tigrcorn.protocols.http2.codec import FRAME_HEADERS, FRAME_SETTINGS, FrameBuffer, FrameWriter, decode_settings, serialize_settings
...
async def _start_server(*, http_versions: list[str], transport: str = 'tcp', seen: dict):
    ...
    kwargs = {'host': '127.0.0.1', 'port': 0, 'lifespan': 'off', 'http_versions': http_versions, 'config': {'http': {'trailer_policy': 'strict'}}}
    if transport == 'udp':
...
class TrailerPolicyStrictLocalTests(unittest.IsolatedAsyncioTestCase):
    async def test_http11_invalid_request_trailer_returns_400(self):
        ...
        try:
            ...
            self.assertTrue(seen['dispatched'])
            writer.close(); await writer.wait_closed()
        finally:
            await server.close()

    async def test_http2_invalid_request_trailer_returns_400(self):
        seen = {'dispatched': False}
        ...
        try:
            ...
            buf = FrameBuffer(); response_headers=[]
            while not response_headers:
                ...
                for frame in buf.pop_all():
                    if frame.frame_type == FRAME_SETTINGS:
                        ...
                        break
            self.assertIn((b':status', b'400'), response_headers)
            self.assertFalse(seen['dispatched'])
            writer.close(); await writer.wait_closed()
        finally:
            await server.close()

    async def test_http3_invalid_request_trailer_returns_400(self):
        seen = {'dispatched': False}
        server, port = await _start_server(http_versions=['3'], transport='udp', seen=seen)
        sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        sock.setblocking(False)
        client = QuicConnection(is_client=True, secret=b'shared', local_cid=b'cli-strict-trailer')
        ...
        try:
            ...
            self.assertIn((b':status', b'400'), response_state.headers)
            self.assertFalse(seen['dispatched'])
        finally:
            sock.close()
            await server.close()
