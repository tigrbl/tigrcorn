...
from tigrcorn.protocols.http3.codec import (
    FRAME_DATA as H3_FRAME_DATA,
    FRAME_HEADERS as H3_FRAME_HEADERS,
    encode_frame as encode_h3_frame,
)
...
@pytest.mark.asyncio
async def test_http11_request_trailers_are_exposed():
    seen: dict = {}
    ...
    assert seen["events"][3]["type"] == "http.disconnect"


@pytest.mark.asyncio
async def test_http2_request_trailers_are_exposed():
    seen: dict = {}
    ...
    assert "tigrcorn.http.request_trailers" in seen["extensions"]
    assert seen["events"][0]["type"] == "http.request"
    assert seen["events"][0]["body"] == b"hello"
    assert not seen["events"][0]["more_body"]
    assert seen["events"][1]["type"] == "http.request.trailers"
    assert seen["events"][1]["trailers"] == [
        (b"x-trailer-one", b"yes"),
        (b"x-trailer-two", b"done"),
    ]
    assert seen["events"][2]["type"] == "http.disconnect"


@pytest.mark.asyncio
async def test_http3_request_trailers_are_exposed():
    seen: dict = {}
    ...
    try:
        ...
        trailer_block = core.encode_headers(
            0, [(b"x-trailer-one", b"yes"), (b"x-trailer-two", b"done")]
        )
