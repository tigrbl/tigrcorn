...
async def _start_server(*, http_versions: list[str], policy: str, transport: str = "tcp"):
    ...
    kwargs = {
        ...
        "config": {"http": {"content_coding_policy": policy}},
    }
    if transport == "udp":
...
@pytest.mark.asyncio
async def test_http11_identity_only_forbidden_returns_406() -> None:
...
@pytest.mark.asyncio
async def test_http11_strict_unsupported_encoding_returns_406() -> None:
...
@pytest.mark.asyncio
async def test_http2_identity_only_forbidden_returns_406() -> None:
    ...
    try:
        ...
        while not ended:
            ...
            if response_headers and ended:
                break
        assert (b":status", b"406") in response_headers
        assert (b"vary", b"accept-encoding") in response_headers
        assert bytes(body) == b"not acceptable"
        writer.close()
        await writer.wait_closed()
    finally:
        await server.close()
...
@pytest.mark.asyncio
async def test_http2_strict_unsupported_encoding_returns_406() -> None:
...
@pytest.mark.asyncio
async def test_http3_identity_only_forbidden_returns_406() -> None:
    ...
    try:
        ...
        assert (b":status", b"406") in response_state.headers
        assert (b"vary", b"accept-encoding") in response_state.headers
        assert response_state.body == b"not acceptable"
    finally:
        sock.close()
        await server.close()
...
@pytest.mark.asyncio
async def test_http3_strict_unsupported_encoding_returns_406() -> None:
