...
class Phase4AdvancedDeliveryUnitTests(unittest.TestCase):
    def test_runtime_compatibility_matrix_is_honest(self):
        matrix = runtime_compatibility_matrix()
        self.assertEqual(set(matrix), {'auto', 'asyncio', 'uvloop'})
        self.assertTrue(matrix['auto']['implemented'])
        self.assertNotIn('trio', matrix)

    def test_public_runtime_surface_descopes_trio(self):
        parser = build_parser()
        ...
        self.assertEqual(json.loads(matrix_path.read_text()), runtime_compatibility_matrix())

    def test_alt_svc_auto_values_resolve_and_suppress_on_http3(self):
    ...
    def test_phase4_artifacts_and_examples_exist(self):
        expected = [
    ...
    def test_phase4_status_json_is_honest(self):
        ...
        self.assertFalse(payload['boundary']['expanded_program_fully_featured'])
        self.assertFalse(payload['boundary']['expanded_program_fully_rfc_compliant'])
        self.assertEqual(payload['implemented']['runtime_embedding']['trio_surface'], 'descoped_not_supported')
        self.assertEqual(payload['implemented']['runtime_embedding']['supported_runtimes'], ['auto', 'asyncio', 'uvloop'])


class StaticFilesPhase4Tests(unittest.IsolatedAsyncioTestCase):
    async def test_static_files_precompressed_sidecars_and_range_identity_coexist(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(sent[1]['body'], encoded)

            sent.clear()
            await app({'type': 'http', 'method': 'HEAD', 'path': '/hello.txt', 'headers': [(b'accept-encoding', b'gzip')]}, receive, send)
            self.assertEqual(sent[0]['status'], 200)
            headers = dict(sent[0]['headers'])
            self.assertEqual(headers[b'content-length'], str(len(encoded)).encode('ascii'))
            ...
            self.assertEqual(headers[b'content-range'], b'bytes 6-10/31')
            self.assertEqual(sent[1]['body'], b'world')


class Phase4AdvancedDeliveryIntegrationTests(unittest.IsolatedAsyncioTestCase):
    async def test_embedded_server_context_manager_runs_hooks(self):
        events: list[str] = []
        ...
        async with EmbeddedServer(app, config) as embedded:
            ...
            self.assertEqual(body, b'embedded')
            writer.close()
            await writer.wait_closed()
        self.assertEqual(events, ['startup', 'shutdown'])

    async def test_http11_early_hints_and_alt_svc_auto(self):
        async def app(scope, receive, send):
            await receive()
        ...
        try:
            ...
            self.assertEqual(final_headers[b'alt-svc'], f'h3=":{udp_port}"; ma=86400'.encode('ascii'))
            writer.close()
            await writer.wait_closed()
        finally:
            await server.close()

    async def test_http2_early_hints_and_alt_svc_auto(self):
        async def app(scope, receive, send):
            await receive()
        ...
        try:
            ...
            self.assertIn((b':status', b'103'), interim)
            self.assertIn((b'link', b'</app.css>; rel=preload; as=style'), interim)
            self.assertNotIn((b'x-unsafe', b'no'), interim)
            self.assertIn((b':status', b'200'), final)
            self.assertIn((b'alt-svc', f'h3=":{udp_port}"; ma=86400'.encode('ascii')), final)
    ...
    async def test_http3_early_hints_are_preserved_before_final_headers(self):
        async def app(scope, receive, send):
            await receive()
            await send({'type': 'http.response.start', 'status': 103, 'headers': [(b'link', b'</app.js>; rel=preload; as=script'), (b'x-unsafe', b'no')]})
            await send({'type': 'http.response.start', 'status': 200, 'headers': [(b'content-type', b'text/plain')]})
