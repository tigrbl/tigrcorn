...
def test_transport_parameters_binary_roundtrip_preserves_extensions():
...
def test_server_handshake_flight_is_split_across_initial_and_handshake_spaces():
...
def test_crypto_reassembly_waits_for_missing_prefix():
    cert_pem, key_pem = generate_self_signed_certificate('server.example')
    ...
    assert server.take_handshake_datagrams()
def test_key_update_allows_post_update_short_packets():
...
def test_packet_number_spaces_are_isolated():
...
def test_persistent_congestion_collapses_congestion_window():
    recovery = QuicLossRecovery(max_datagram_size=1200)
...
def test_pacing_budget_replenishes_after_time_passes():
    recovery = QuicLossRecovery(max_datagram_size=1200)
