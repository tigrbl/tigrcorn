...
class Phase9HPromotionEvaluatorHardeningTests(unittest.TestCase):
    ...
    def test_actual_repository_performance_section_passes_under_hardened_evaluator(self) -> None:
    ...
    def test_missing_metric_key_fails(self) -> None:
        root = self._copy_performance_tree()
        try:
            ...
            self.assertIn('performance artifacts are missing required SLO metric keys', '\n'.join(report.failures))
        finally:
            shutil.rmtree(root)

    def test_missing_threshold_key_fails(self) -> None:
        root = self._copy_performance_tree()
        try:
            matrix_file = root / 'docs/review/performance/performance_matrix.json'
            matrix = self._load_json(matrix_file)
            matrix['profiles'][0]['thresholds'].pop('max_p99_9_ms', None)
            matrix_file.write_text(json.dumps(matrix, indent=2, sort_keys=False) + '\n', encoding='utf-8')
            report = _evaluate_performance_target(root, PERFORMANCE_CONFIG)
            self.assertFalse(report.passed)
            self.assertIn('http11_baseline missing required threshold keys', '\n'.join(report.failures))
        finally:
            shutil.rmtree(root)

    def test_missing_relative_budget_key_fails(self) -> None:
        root = self._copy_performance_tree()
        try:
            matrix_file = root / 'docs/review/performance/performance_matrix.json'
            matrix = self._load_json(matrix_file)
            ...
            report = _evaluate_performance_target(root, PERFORMANCE_CONFIG)
            self.assertFalse(report.passed)
            self.assertIn('http11_baseline missing required relative regression budget keys', '\n'.join(report.failures))
        finally:
            shutil.rmtree(root)

    def test_missing_root_artifact_file_fails(self) -> None:
        root = self._copy_performance_tree()
        try:
            ...
            self.assertFalse(report.passed)
            failures = '\n'.join(report.failures)
            self.assertIn('missing performance summary file', failures)
            self.assertIn('performance artifact root is missing required files', failures)
        finally:
            shutil.rmtree(root)

    def test_missing_profile_artifact_file_fails(self) -> None:
        root = self._copy_performance_tree()
        try:
            ...
            self.assertFalse(report.passed)
            failures = '\n'.join(report.failures)
            self.assertIn('missing artifact file for http11_baseline', failures)
            self.assertIn('http11_baseline performance artifact directory is missing required files', failures)
        finally:
            shutil.rmtree(root)
    ...
    def test_missing_required_lane_fails(self) -> None:
        ...
        try:
            ...
            self.assertFalse(report.passed)
            failures = '\n'.join(report.failures)
            self.assertIn('performance matrix is missing required lanes', failures)
            self.assertIn('performance artifact summary is missing required lane counts', failures)
        finally:
            shutil.rmtree(root)

    def test_missing_certification_platform_declaration_fails(self) -> None:
        root = self._copy_performance_tree()
        try:
            matrix_file = root / 'docs/review/performance/performance_matrix.json'
            matrix = self._load_json(matrix_file)
            ...
            env_payload = self._load_json(env_file)
            ...
            self.assertFalse(report.passed)
            failures = '\n'.join(report.failures)
            self.assertIn('performance matrix metadata is missing certification_platforms declarations', failures)
            self.assertIn('http11_baseline missing profile certification_platforms declarations in matrix', failures)
            self.assertIn('http11_baseline missing env.json certification_platform declaration', failures)
        finally:
            shutil.rmtree(root)

    def test_missing_rfc_correctness_checks_fails(self) -> None:
        root = self._copy_performance_tree()
        try:
            correctness_file = root / 'docs/review/performance/artifacts/phase6_current_release/http11_baseline/correctness.json'
            correctness = self._load_json(correctness_file)
            ...
            self.assertFalse(report.passed)
            failures = '\n'.join(report.failures)
            self.assertIn('http11_baseline correctness.json is not marked required=true for an RFC-scoped profile', failures)
            self.assertIn('http11_baseline correctness.json is missing correctness checks for an RFC-scoped profile', failures)
        finally:
            shutil.rmtree(root)

    def test_missing_live_listener_metadata_fails(self) -> None:
        root = self._copy_performance_tree()
        try:
            command_file = root / 'docs/review/performance/artifacts/phase6_current_release/http11_baseline/command.json'
            payload = self._load_json(command_file)
            ...
            self.assertFalse(report.passed)
            failures = '\n'.join(report.failures)
            self.assertIn('http11_baseline command.json does not preserve live_listener_required=true', failures)
            self.assertIn('http11_baseline command.json does not preserve lane="end_to_end_release"', failures)
        finally:
            shutil.rmtree(root)
