...
@pytest.mark.asyncio
async def test_startup_and_shutdown_hooks_are_ordered_relative_to_lifespan() -> None:
    events: list[str] = []
    ...
    async def on_start(server) -> None:
        assert server.lifespan.started
    ...
    assert events == ['lifespan.startup', 'hook.startup', 'lifespan.shutdown', 'hook.shutdown']

@pytest.mark.asyncio
async def test_startup_hook_failures_abort_startup() -> None:
...
@pytest.mark.asyncio
async def test_shutdown_hook_failures_are_suppressed_during_close() -> None:
    events: list[str] = []
    ...
    await server.start()
    await server.close()
    assert events == ['hook.shutdown']


def test_reload_hooks_run_before_child_restart_and_receive_config() -> None:
    events: list[str] = []
    config = build_config(app='tests.fixtures_pkg.appmod:app', host='127.0.0.1', port=8000)

    async def on_reload(reload_config) -> None:
        assert reload_config is config
    ...
    assert events == ['hook.reload', 'stop_child', 'spawn_child']


@pytest.mark.asyncio
async def test_embedded_server_is_a_first_class_documented_surface() -> None:
    config = build_config(host='127.0.0.1', port=0, lifespan='off')
    ...
    assert embedded.bound_endpoints()
    await embedded.close()

def test_lifecycle_and_embedded_server_docs_exist() -> None:
    readme = Path('README.md').read_text(encoding='utf-8')
    contract = Path('docs/LIFECYCLE_AND_EMBEDDED_SERVER.md').read_text(encoding='utf-8')
    assert 'docs/LIFECYCLE_AND_EMBEDDED_SERVER.md' in readme
    assert 'on_startup' in contract
    ...
    assert 'lifespan.shutdown()' in contract
