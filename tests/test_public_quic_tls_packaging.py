...
class PublicQuicTlsPackagingTests(unittest.TestCase):
    def test_client_initial_datagrams_meet_the_rfc_minimum_size(self):
        client = QuicConnection(is_client=True, secret=DEFAULT_QUIC_SECRET, local_cid=b'cli1cli1')
        self.assertGreaterEqual(len(client.build_initial()), 1200)
        ...
        self.assertGreaterEqual(len(client.start_handshake()), 1200)


class PublicHTTP3ServerWithQuicTlsTests(unittest.IsolatedAsyncioTestCase):
    async def test_http3_roundtrip_works_with_public_udp_tls_config(self):
    ...
    async def test_http3_roundtrip_supports_quic_client_auth(self):
        async def app(scope, receive, send):
            self.assertEqual(scope['type'], 'http')
            self.assertEqual(scope['http_version'], '3')
            event = await receive()
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                assert response_state is not None
                self.assertIn((b':status', b'200'), response_state.headers)
                self.assertEqual(response_state.body, b'mtls:hello')
            finally:
                sock.close()
                await server.close()

    async def test_http3_websocket_roundtrip_works_with_public_udp_tls_config(self):
        seen = {}
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                self.assertEqual(seen['text'], 'hello-h3-tls')
                ...
                self.assertEqual(message_frame.payload.decode('utf-8'), 'hello-h3-tls')
