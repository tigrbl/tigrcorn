...
async def test_scheduler_limit_concurrency_is_real_global_inflight_cap():
    scheduler = ProductionScheduler(SchedulerPolicy(limit_concurrency=1))
    first = scheduler.acquire_work()
    assert first is not None
    assert first is not None
    assert scheduler.current_inflight == 1
    assert scheduler.acquire_work() is None
    first.release()
    assert scheduler.current_inflight == 0
    gate = asyncio.Event()
...
async def test_http11_limit_concurrency_returns_503_on_second_request():
    ...
    try:
        ...
        assert b'scheduler overloaded' in await asyncio.wait_for(r2.read(), 1.0)
        assert server.state.metrics.scheduler_rejections >= 1
        release.set()
...
async def test_http2_limit_concurrency_returns_503_on_second_stream():
    release = asyncio.Event()
    started = asyncio.Event()
    ...
    try:
        ...
        assert statuses[3] == b'503'
        assert b'scheduler overloaded' in bytes(bodies[3])
        assert server.state.metrics.scheduler_rejections >= 1
        release.set()
        writer.close()
...
async def test_http3_websocket_admission_returns_503_when_limit_reached():
    async def app(scope, receive, send):
    ...
    try:
        ...
        assert first_response is not None
        assert (b':status' in b'200'), first_response.headers
        ...
        assert second_response is not None
        assert (b':status' in b'503'), second_response.headers
        assert second_response.body == b'scheduler overloaded'
        assert server.state.metrics.scheduler_rejections >= 1
    finally:
        sock.close()
        await server.close()

async def test_http11_websocket_keepalive_sends_ping_then_closes_on_timeout():
    async def app(scope, receive, send):
        if scope['type'] != 'websocket':
            return
    ...
    try:
        ...
        assert ping.opcode == 0x9
        assert close.opcode == 0x8
        code, reason = decode_close_payload(close.payload)
        assert code == 1011
        assert reason == 'ping timeout'
        assert server.state.metrics.websocket_pings_sent >= 1
        assert server.state.metrics.websocket_ping_timeouts >= 1
        writer.close()
        await writer.wait_closed()
    finally:
        await server.close()

async def test_http2_websocket_keepalive_sends_ping_then_closes_on_timeout():
    async def app(scope, receive, send):
        if scope['type'] != 'websocket':
            return
        connect = await receive()
...
async def test_http3_websocket_keepalive_sends_ping_then_closes_on_timeout():
    async def app(scope, receive, send):
        if scope['type'] != 'websocket':
            return
        connect = await receive()
