...
class StaticAndPathsendSurfaceTests(unittest.IsolatedAsyncioTestCase):
    async def test_static_files_app_prefers_standard_pathsend_when_advertised(self):
    ...
    async def test_mount_static_app_routes_requests_and_preserves_fallback(self):
        ...
        async def fallback(scope, receive, send) -> None:
            fallback_events.append({'scope_type': scope['type'], 'path': scope.get('path'), 'root_path': scope.get('root_path', '')})
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            await app({'type': 'http', 'method': 'GET', 'path': '/assets/hello.txt', 'raw_path': b'/assets/hello.txt', 'headers': []}, receive, send_static)
            self.assertEqual(sent_static[0]['status'], 200)
            self.assertEqual(sent_static[1]['type'], 'http.response.body')
            self.assertEqual(sent_static[1]['body'], b'static hello')
            ...
            await app({'type': 'http', 'method': 'GET', 'path': '/api', 'raw_path': b'/api', 'headers': []}, receive, send_fallback)
            self.assertEqual(sent_fallback[0]['status'], 200)
            self.assertEqual(sent_fallback[1]['body'], b'fallback')
    ...
    async def test_http11_pathsend_round_trip(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            async def app(scope, receive, send):
                self.assertIn('http.response.pathsend', scope.get('extensions', {}))
                await receive()
                await send({'type': 'http.response.start', 'status': 200, 'headers': [(b'content-type', b'application/octet-stream'), (b'content-length', str(len(payload)).encode('ascii'))]})
    ...
    async def test_http2_pathsend_round_trip(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                self.assertEqual(dict(response_headers)[b':status'], b'200')
                self.assertEqual(body, payload)
                writer.close()
                await writer.wait_closed()
            finally:
    ...
    async def test_cli_main_allows_static_only_mount_without_app_import_string(self):
        ...
        self.assertEqual(config.static.mount, str(root))
        self.assertEqual(config.static.expires, 60)
        self.assertIsNone(config.app.target)
