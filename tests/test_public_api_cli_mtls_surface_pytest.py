...
async def test_serve_builds_config_with_client_certificate_options():
    config = object()
...
async def test_serve_import_string_forwards_client_certificate_options():
    serve = AsyncMock()
    with (
...
def test_run_with_import_string_forwards_client_certificate_options():
...
def test_run_with_app_instance_forwards_client_certificate_options():
    real_asyncio_run = asyncio.run
    serve = AsyncMock()
    with (
        patch('tigrcorn.api.serve', new=serve),
    ...
    assert kwargs['ssl_ca_certs'] == 'client-ca.pem'
    assert kwargs['ssl_require_client_cert']
def test_cli_main_forwards_client_certificate_options():
