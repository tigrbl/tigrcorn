...
class QuicAdditionalRFCTests(unittest.TestCase):
    def test_send_offsets_advance_across_packets(self):
        ...
        self.assertEqual(stream1.data, b'hello')
        self.assertEqual(stream2.data, b'world')
        self.assertTrue(stream2.fin)

    def test_out_of_order_stream_data_is_reassembled(self):
        state = QuicStreamState(0)
        self.assertEqual(state.apply(QuicStreamFrame(stream_id=0, offset=5, data=b'world', fin=True)), b'')
        self.assertEqual(state.apply(QuicStreamFrame(stream_id=0, offset=0, data=b'hello')), b'helloworld')
        self.assertTrue(state.received_final)


class HTTP3AdditionalRFCTests(unittest.IsolatedAsyncioTestCase):
        ...
        try:
            ...
            for _ in range(2):
                ...
                if control_event is not None:
                    break
            self.assertIsNotNone(control_event)
            assert control_event is not None
            self.assertEqual(control_event.stream_id, 3)
            self.assertFalse(control_event.fin)
        finally:
            sock.close()
            await server.close()
    ...
    async def test_connection_core_accepts_control_stream_on_server_uni_id(self):
        ...
        self.assertIsNone(receiver.receive_stream_data(3, payload, fin=False))
        self.assertEqual(receiver.state.remote_control_stream_id, 3)
        self.assertEqual(receiver.state.remote_settings, {1: 0, 6: 1200})
