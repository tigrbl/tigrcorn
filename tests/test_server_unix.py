...
class UnixServerTests(unittest.IsolatedAsyncioTestCase):
    async def test_unix_http_roundtrip(self):
        async def app(scope, receive, send):
            ...
            self.assertEqual(scope['path'], '/unix')
            await send({'type': 'http.response.start', 'status': 200, 'headers': [(b'content-type', b'text/plain')]})
            await send({'type': 'http.response.body', 'body': b'ok:' + event['body'], 'more_body': False})
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                writer.write(b'POST /unix HTTP/1.1\r\nHost: localhost\r\nContent-Length: 3\r\n\r\nhey')
                await writer.drain()
                data = await reader.read(65535)
                self.assertIn(b'200 OK', data)
                self.assertTrue(data.endswith(b'ok:hey'))
                writer.close()
