...
def test_http_integrity_caching_signatures_status_document_exists() -> None:
    ...
    assert rfc_status['rfc7232']['current_support'] == 'supported_and_targeted'
    assert rfc_status['rfc7232']['targeted_by_authoritative_boundary'] is True
    assert rfc_status['rfc7233']['current_support'] == 'supported_and_targeted'
    ...
    assert feature_status['etag']['current_support'] == 'supported'
    assert feature_status['if_none_match']['current_support'] == 'supported'
    assert feature_status['range']['current_support'] == 'supported'
    assert feature_status['if_range']['current_support'] == 'supported'
    assert feature_status['response_304']['current_support'] == 'supported_by_conditional_engine'
