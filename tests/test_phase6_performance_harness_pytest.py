...
def test_matrix_declares_all_required_profile_ids() -> None:
...
def test_preserved_artifacts_validate_for_current_release() -> None:
    ...
    assert failures == []
def test_each_profile_has_required_artifact_files() -> None:
...
def test_each_profile_links_to_a_known_deployment_profile() -> None:
    matrix = load_performance_matrix(MATRIX_PATH)
...
def test_rfc_scoped_profiles_require_correctness_checks() -> None:
    matrix = load_performance_matrix(MATRIX_PATH)
    for profile in matrix.profiles:
        if profile.rfc_targets:
            assert profile.correctness_required, profile.profile_id
def test_matrix_declares_required_lanes_and_platforms() -> None:
    matrix = load_performance_matrix(MATRIX_PATH)
...
def test_root_artifact_summary_declares_platform_and_lanes() -> None:
...
def test_rfc_scoped_profile_artifacts_record_correctness_requirements() -> None:
    matrix = load_performance_matrix(MATRIX_PATH)
    for profile in matrix.profiles:
...
def test_end_to_end_release_profiles_preserve_live_listener_metadata() -> None:
    matrix = load_performance_matrix(MATRIX_PATH)
    for profile in matrix.profiles:
...
def test_runner_can_execute_a_selected_profile_into_a_temp_root() -> None:
