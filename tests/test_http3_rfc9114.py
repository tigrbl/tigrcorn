...
class HTTP3RFC9114Tests(unittest.TestCase):
    def test_control_stream_prefix_and_settings(self):
        ...
        self.assertEqual(stream_type, STREAM_TYPE_CONTROL)
        frame, _ = decode_frame(payload, offset)
        self.assertEqual(frame.frame_type, FRAME_SETTINGS)
        self.assertEqual(decode_settings(frame.payload), {1: 0, 6: 1200})

    def test_goaway_uses_varint_payload(self):
        core = HTTP3ConnectionCore()
        raw = core.encode_goaway(33)
        frame, _ = decode_frame(raw, 0)
        self.assertEqual(frame.frame_type, FRAME_GOAWAY)
        stream_id, _ = decode_quic_varint(frame.payload, 0)
        self.assertEqual(stream_id, 33)
