...
def test_peer_initiated_stream_limit_is_enforced():
    client, server = _pair()
...
def test_unidirectional_stream_role_is_enforced():
    client, server = _pair()
    server.receive_datagram(client.send_stream_data(2, b'hello', fin=True))
    with pytest.raises(ProtocolError):
        server.send_stream_data(2, b'reply')

def test_distinct_bidi_local_bidi_remote_and_uni_send_windows_are_applied():
    ...
    assert isinstance(server.send_stream_data(0, b'abc'), bytes)
    ...
    assert isinstance(server.send_stream_data(1, b'abcdefg'), bytes)
    ...
    assert isinstance(server.send_stream_data(3, b'abcdefghijk'), bytes)
def test_receive_side_flow_control_rejects_stream_data_beyond_local_limits():
    client, server = _pair()
...
def test_reset_stream_final_size_counts_toward_receive_flow_control():
    client, server = _pair()
    server.flow.configure_local_initial_limits(
        max_data=4,
...
def test_stop_sending_queues_reset_stream():
    client, server = _pair()
    server.receive_datagram(client.send_stream_data(0, b'abc', fin=False))
    stop_events = client.receive_datagram(server.stop_sending(0, 99))
    assert any(event.kind == 'stop_sending' and event.stream_id == 0 for event in stop_events)
    pending = client.take_pending_datagrams()
    assert len(pending) == 1
    reset_events = server.receive_datagram(pending[0])
    assert any(event.kind == 'reset_stream' and event.stream_id == 0 for event in reset_events)
def test_closed_peer_stream_recycles_max_streams_credit():
    client, server = _pair()
    server.streams.configure_local_initial_limits(bidirectional=1, unidirectional=0)
    ...
    assert len(max_stream_events) == 1
    assert max_stream_events[0].detail.bidirectional
    assert max_stream_events[0].detail.maximum_streams == 2
def test_ack_delay_exponent_is_used_when_encoding_ack_frames():
    _client, server = _pair()
    ...
    assert ack.ack_delay >= 1500
def test_credit_connection_and_stream_expand_local_receive_limits_only():
    _client, server = _pair()
    send_limit = server.flow.connection_window
    local_limit = server.flow.local_connection_window
    server.credit_connection(10)
    assert server.flow.connection_window == send_limit
    assert server.flow.local_connection_window == local_limit + 10
    ...
    assert server.flow.window_for_stream(0) == stream_send_limit
    assert server.flow.receive_window_for_stream(0) == stream_recv_limit + 5
