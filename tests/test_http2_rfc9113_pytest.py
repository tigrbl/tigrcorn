...
def test_reject_duplicate_pseudo_header() -> None:
    handler = _handler()
...
def test_reject_pseudo_after_regular() -> None:
    handler = _handler()
    state = H2StreamState(1)
    state.headers = [
        (b":method", b"GET"),
...
def test_reject_invalid_connection_header() -> None:
    handler = _handler()
    state = H2StreamState(1)
    state.headers = [
        (b":method", b"GET"),
...
def test_reject_invalid_te_header() -> None:
    handler = _handler()
    state = H2StreamState(1)
    state.headers = [
        (b":method", b"GET"),
...
def test_reject_uppercase_header_field_name() -> None:
    handler = _handler()
    state = H2StreamState(1)
    state.headers = [
        (b":method", b"GET"),
...
def test_reject_even_numbered_request_stream() -> None:
    handler = _handler()
    frame = HTTP2Frame(
        frame_type=FRAME_HEADERS, flags=FLAG_END_HEADERS, stream_id=2, payload=b""
    )
    with pytest.raises(ProtocolError):
        asyncio.run(handler._handle_headers(frame))


def test_reject_data_before_headers() -> None:
    handler = _handler()
...
def test_reject_invalid_settings_values() -> None:
    with pytest.raises(ProtocolError):
