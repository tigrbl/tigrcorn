...
def test_proxy_scope_building_applies_forwarded_headers():
    ...
    assert scope['client'] == ('203.0.113.8', 50000)
    assert scope['scheme'] == 'https'
    assert scope['server'] == ('example.com', 8000)
    assert scope['root_path'] == '/svc'
    assert scope['path'] == '/hello'
def test_websocket_scope_inherits_proxy_normalization():
    request = ParsedRequest(
        method='GET',
        target='/mount/ws',
        path='/mount/ws',
    ...
    assert scope['client'] == ('198.51.100.10', 50001)
    assert scope['scheme'] == 'wss'
    assert scope['server'] == ('example.net', 8000)
...
def test_metrics_snapshot_and_exporters():
    ...
    assert snapshot['active_websocket_connections'] == 1
    assert 'tigrcorn_requests_served' in metrics.render_prometheus()
    assert 'tigrcorn.requests_served' in metrics.render_statsd()
def test_configure_logging_supports_structured_and_file_handlers():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for handler in logger.handlers:
            handler.flush()
        assert Path(LoggingCfg.access_log_file).exists()
        payload = Path(LoggingCfg.access_log_file).read_text(encoding='utf-8')
        assert '"event": "access.http"' in payload
def test_local_and_process_workers_expose_health():
    local = LocalWorker()
    local.start()
    assert local.health()['alive']
    local.stop()
    worker = ProcessWorker(name='phase4-test-worker')
    worker.start(_sleep_worker, 10.0)
    try:
        assert worker.is_alive()
        assert worker.health()['pid'] is not None
    finally:
        worker.stop(timeout=0.5)

def test_worker_supervisor_can_replace_local_worker():
...
def test_polling_reloader_detects_snapshot_changes():
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        source = root / 'app.py'
        ...
        assert snap1 != snap2
def test_server_supervisor_restarts_dead_workers():
    parser = build_parser()
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with patch('tigrcorn.server.supervisor.run_worker_from_config_payload', _noop_worker):
            ...
            try:
                ...
                assert len(supervisor.workers.workers) == 1
            finally:
                supervisor.stop()
...
def test_cli_parser_still_accepts_operator_surface_flags():
    ...
    assert ns.metrics_bind == '127.0.0.1:9001'
    assert ns.max_connections == 10
    assert ns.max_streams == 3