...
class HTTP1RFC9112ParserTests(unittest.IsolatedAsyncioTestCase):
    async def test_absolute_form_request_target(self):
        ...
        self.assertEqual(request.path, '/alpha')
        self.assertEqual(request.query_string, b'x=1')
        self.assertEqual(request.target_form, 'absolute')

    async def test_asterisk_form_restricted_to_options(self):
        reader = asyncio.StreamReader()
    ...
    async def test_missing_host_rejected_for_http11(self):
        reader = asyncio.StreamReader()
        reader.feed_data(b'GET / HTTP/1.1\r\n\r\n')
        reader.feed_eof()
        with self.assertRaises(ProtocolError):
    ...
    async def test_unsupported_transfer_encoding_chain_rejected(self):
        reader = asyncio.StreamReader()
        reader.feed_data(
...
class HTTP1RFC9112ServerTests(unittest.IsolatedAsyncioTestCase):
    async def test_expect_continue_sent_on_first_receive(self):
        async def app(scope, receive, send):
            self.assertEqual(scope['path'], '/upload')
            event = await receive()
            self.assertEqual(event['body'], b'hello')
            self.assertFalse(event['more_body'])
    ...
    async def test_chunked_request_body_is_streamed_as_multiple_events(self):
        ...
        try:
            ...
            self.assertEqual(seen_events, [(b'hel', True), (b'lo', True), (b'', False)])
            writer.close()
            await writer.wait_closed()
        finally:
            await server.close()
