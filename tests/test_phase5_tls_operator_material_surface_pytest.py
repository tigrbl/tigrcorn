...
def _write_tls_materials(root: Path) -> tuple[Path, Path, Path, Path, Path, x509.Certificate]:
...
def test_cli_and_env_wiring_accept_ssl_keyfile_password_and_ssl_crl() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        cert_path, key_path, _encrypted_key_path, _ca_path, crl_path, _client_cert = _write_tls_materials(root)
        ...
        assert config.tls.keyfile_password == 'cli-secret'
        assert config.tls.crl == str(crl_path)
        assert config.listeners[0].ssl_keyfile_password == 'cli-secret'
        assert config.listeners[0].ssl_crl == str(crl_path)
        assert config.listeners[0].crl_mode == 'soft-fail'
        ...
        assert env_config.tls.keyfile_password == 'env-secret'
        assert env_config.tls.crl == str(crl_path)
        assert env_config.listeners[0].ssl_keyfile_password == 'env-secret'
        assert env_config.listeners[0].ssl_crl == str(crl_path)

def test_encrypted_private_key_material_loads_through_server_tls_context() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        cert_path, _key_path, encrypted_key_path, _ca_path, _crl_path, _client_cert = _write_tls_materials(root)
        ...
        assert context is not None
        assert context.private_key_password == b'hunter2'
        ...
        assert driver._private_key is not None

def test_local_crl_material_is_loaded_and_revoked_client_cert_is_rejected() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        cert_path, key_path, _encrypted_key_path, ca_path, crl_path, client_cert = _write_tls_materials(root)
        listener = ListenerConfig(
            kind='tcp',
            host='127.0.0.1',
            port=4433,
        ...
        assert policy.revocation_mode == RevocationMode.REQUIRE
        assert len(policy.revocation_material.crls) == 1
