...
async def _read_http1_response(
...
def test_generated_etag_parses_and_strong_matches():
    tag = generate_entity_tag(b'hello world')
    parsed = parse_entity_tag(tag)
    assert parsed is not None
    assert strong_compare(parsed, parse_entity_tag(tag))


def test_conditional_engine_supports_if_none_match_if_match_and_dates():
    ...
    ius = apply_conditional_request(
        ...
        request_headers=[
            (
                ...
                formatdate(1_699_999_000, usegmt=True).encode('ascii'),
            )
        ],
        response_headers=[(b'last-modified', last_modified)],
        body=b'payload',
...
def test_range_engine_supports_single_multi_and_unsatisfied_ranges():
    body = b'hello world'
...
def test_entity_semantics_head_and_range_interactions():
    ...
    assert head.body == b''
    assert get_header(head.headers, b'content-length') == b'5'
    assert get_header(head.headers, b'etag') is not None
    ...
    assert partial.body == b'hello'
    assert get_header(partial.headers, b'content-encoding') is None


@pytest.mark.asyncio
async def test_static_files_app_supports_etag_conditionals_and_ranges():
...
@pytest.mark.asyncio
async def test_http11_generates_etag_and_honors_if_none_match():
...
@pytest.mark.asyncio
async def test_http2_head_preserves_content_length_and_suppresses_body():
    async def app(scope, receive, send):
        await receive()
        await send(
            {
    ...
    try:
        ...
        assert (b'content-length', b'5') in response_headers
        assert body == b''
        assert dict(response_headers).get(b'etag') is not None
        writer.close()
        await writer.wait_closed()
    finally:
        await server.close()
...
@pytest.mark.asyncio
async def test_http3_range_request_returns_partial_response():
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        (root / 'hello.txt').write_text('hello world', encoding='utf-8')
        app = StaticFilesApp(root)
        ...
        try:
            ...
            assert (b':status', b'206') in response_headers
            assert (b'content-range', b'bytes 6-10/11') in response_headers
            assert body == b'world'
        finally:
            sock.close()
            await server.close()
