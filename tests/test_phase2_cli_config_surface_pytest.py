...
def test_parser_accepts_grouped_phase2_flags():
    parser = build_parser()
...
def test_build_config_from_namespace_maps_nested_submodels():
    parser = build_parser()
    ns = parser.parse_args(
        [
            'tests.fixtures_pkg.appmod:app',
...
def test_config_source_precedence_cli_over_env_over_file():
    parser = build_parser()
...
def test_app_dir_round_trip():
    parser = build_parser()
    ns = parser.parse_args(['tests.fixtures_pkg.appmod:app', '--app-dir', '.'])
    config = build_config_from_namespace(ns)
    assert config.app.app_dir == '.'
def test_cli_flag_surface_json_covers_public_parser_flags():
    parser = build_parser()
