...
from tigrcorn.asgi.send import (
        ...
        await send(
            {
                ...
                'body': chunk,
                'more_body': offset + chunk_size < len(PAYLOAD),
            }
        )


@pytest.mark.asyncio
async def test_response_collector_spools_large_body_and_preserves_entity_metadata():
    ...
    assert collector.has_spooled_body()
    assert collector.body_length == 60
    segments = collector.spooled_body_segments()
    assert len(segments) == 1
    assert isinstance(segments[0], FileBodySegment)
    assert segments[0].count == 60
    assert await materialize_response_body_segments(segments) == (b'a' * 20) + (b'b' * 40)
    assert collector.generated_entity_tag().startswith(b'"')
    collector.cleanup()


def test_file_backed_planning_supports_ranges_conditionals_and_materialization_boundary():
    import tempfile
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert ranged.use_body_segments
        assert ranged.status == 206
        assert get_header(ranged.headers, b'content-range') == f'bytes 128-255/{len(payload)}'.encode('ascii')
        ...
        assert not not_modified.use_body_segments
        ...
        assert coded.requires_materialization


async def _assert_streamed_without_materialization(*, http_versions: list[str], transport: str = 'tcp') -> None:
    ...
    with (
        ...
        server, port = await _start_server(_streaming_app, http_versions=http_versions, transport=transport)
        try:
            if transport == 'udp':
            ...
            assert header_map[b'content-length'] == str(len(PAYLOAD)).encode('ascii')
            assert header_map.get(b'etag') is not None
        finally:
            await server.close()


@pytest.mark.asyncio
async def test_http11_generic_large_response_spools_and_streams_without_materialization():
    await _assert_streamed_without_materialization(http_versions=['1.1'])


@pytest.mark.asyncio
async def test_http2_generic_large_response_spools_and_streams_without_materialization():
    await _assert_streamed_without_materialization(http_versions=['2'])


@pytest.mark.asyncio
async def test_http3_generic_large_response_spools_and_streams_without_materialization():
    await _assert_streamed_without_materialization(http_versions=['3'], transport='udp')
