...
def test_cli_and_config_runtime_fields_resolve_ssl_ciphers():
    config = build_config(
        host='127.0.0.1',
        port=0,
    ...
    assert config.tls.resolved_cipher_suites == (0x1301,)
    assert config.listeners[0].ssl_ciphers == 'TLS_AES_128_GCM_SHA256'
    assert config.listeners[0].resolved_cipher_suites == (0x1301,)
def test_invalid_ssl_cipher_expressions_fail_fast():
...
def test_build_server_ssl_context_carries_resolved_cipher_suites():
    config = build_config(
        host='127.0.0.1',
        port=0,
        ssl_certfile='tests/fixtures_certs/interop-localhost-cert.pem',
    ...
    assert context is not None
    assert context.cipher_suites == (0x1301,)
async def test_tcp_tls_negotiated_suite_changes_with_configured_allowlist():
    ...
    try:
        ...
        try:
            assert writer.get_extra_info('ssl_object').cipher()[0] == 'TLS_AES_128_GCM_SHA256'
...
async def test_quic_tls_negotiated_suite_changes_with_configured_allowlist():
    ...
    try:
        ...
        assert client.handshake_driver._selected_cipher_suite == 0x1301
        sock.close()
    finally:
        await server.close()
        tmpdir.cleanup()

def test_phase9f1_status_snapshot_matches_current_flag_surface_state():
    payload = json.loads((CONFORMANCE / 'phase9f1_tls_cipher_policy.current.json').read_text(encoding='utf-8'))
    assert payload['phase'] == '9F1'
    assert payload['implemented_flag'] == '--ssl-ciphers'
    assert '--ssl-ciphers' not in payload['current_state']['remaining_flag_runtime_blockers']
    failures = '\n'.join(evaluate_promotion_target(ROOT).flag_surface.failures)
    assert '--ssl-ciphers' not in failures
