...
from tigrcorn.protocols.websocket.frames import decode_close_payload, encode_frame, read_frame
...
class WebSocketAdditionalRFC6455Tests(unittest.IsolatedAsyncioTestCase):
    async def test_accept_rejects_extension_negotiation(self):
        sender = _WSAppSend(
            writer=type('W', (), {'write': lambda self, data: None, 'drain': staticmethod(asyncio.sleep)})(),
            server_header=None,
            state={
                'accepted': False,
                'closed': False,
        ...
        with self.assertRaises(RuntimeError):
            await sender({'type': 'websocket.accept', 'headers': [(b'sec-websocket-extensions', b'permessage-deflate')]})
    ...
    async def test_invalid_utf8_text_message_closes_with_1007(self):
        ...
        try:
            ...
            self.assertEqual(code, 1007)
            self.assertEqual(reason, 'invalid frame payload data')
            self.assertEqual(disconnects[0]['code'], 1007)
            writer.close()
    ...
    async def test_fragmented_message_limit_closes_with_1009(self):
        ...
        try:
            ...
            self.assertEqual(disconnects[0]['code'], 1009)
            writer.close()
            await writer.wait_closed()
        finally:
            await server.close()
