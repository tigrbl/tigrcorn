...
class TestWebSocketServerTests:
    async def test_websocket_echo(self):
        ...
        try:
            ...
            writer.write(
                encode_frame(opcode=1, payload=b"hello", fin=True, masked=True)
            )
            await writer.drain()
            frame = await read_frame(reader, max_payload_size=1024, expect_masked=False)
            assert frame.payload == b"hello"
            close_frame = await read_frame(
                reader, max_payload_size=1024, expect_masked=False
            )
            assert close_frame.opcode == 8
            writer.close()
