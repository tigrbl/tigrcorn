...
class H3ProbeResult:
    ...
    def to_jsonable(self) -> dict:
        return {
            ...
            "ended": self.ended,
            "preface_events": list(self.preface_events),
        }
    ...
    try:
        ...
        for _attempt in range(4):
            ...
            for event in client.receive_datagram(data):
                preface_events.append(event.kind)
                if event.kind == "stream":
                    core.receive_stream_data(event.stream_id, event.data, fin=event.fin)
            if "ack" in preface_events and "stream" in preface_events:
                break
...
async def _amain(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description="h3 HTTP/3 probe fixture")
    ...
    parser.add_argument("--body", default=os.environ.get("INTEROP_REQUEST_BODY", "hello-h3"))
    args = parser.parse_args(argv)
    if args.version:
        print("h3-http3-client 1.0")
        return 0

    host = os.environ["INTEROP_TARGET_HOST"]
    port = int(os.environ["INTEROP_TARGET_PORT"])
    result = await probe_h3(host, port, method=args.method, path=args.path, body=args.body.encode("utf-8"))
    transcript = {
        "request": {"method": args.method, "path": args.path, "body": args.body},
        "response": result.to_jsonable(),
    }
    negotiation = {"implementation": "tigrcorn-h3-probe", "protocol": "h3", "scheme": "https"}
