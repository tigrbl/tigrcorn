...
class WebSocketRFC6455Tests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_accept_subprotocol_must_be_offered(self):
        ...
        with self.assertRaises(RuntimeError):
            await sender({'type': 'websocket.accept', 'subprotocol': 'superchat', 'headers': []})

    def test_close_frame_valid(self):
        raw = close_frame(1000, 'ok')
        frame = parse_frame_bytes(raw)
        self.assertEqual(frame.opcode, OP_CLOSE)
        code, reason = decode_close_payload(frame.payload)
        self.assertEqual((code, reason), (1000, 'ok'))
