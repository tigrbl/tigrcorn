...
def test_parser_runtime_choices_no_longer_include_trio() -> None:
    parser = build_parser()
    runtime_action = next(
...
def test_config_validation_rejects_trio_runtime_and_worker_class() -> None:
...
def test_runtime_matrix_and_docs_descoped_trio() -> None:
...
def test_direct_runtime_invocation_rejects_trio() -> None:
    with pytest.raises(RuntimeError):
        run_coro_with_runtime(lambda: None, runtime="trio")

def test_promotion_target_remains_green() -> None:
