...
@pytest.mark.asyncio
async def test_parser_rejects_invalid_header_field_name() -> None:
...
@pytest.mark.asyncio
async def test_parser_rejects_invalid_header_field_value() -> None:
    reader = asyncio.StreamReader()
    reader.feed_data(b"GET / HTTP/1.1\r\nHost: example.com\r\nX-Test: bad\x00value\r\n\r\n")
    reader.feed_eof()
    with pytest.raises(ProtocolError):
...
@pytest.mark.asyncio
async def test_http11_emits_informational_response_before_final_response() -> None:
    ...
    try:
        ...
        assert b"103 Early Hints" in interim
        assert b"link: </style.css>; rel=preload; as=style" in interim.lower()
...
@pytest.mark.asyncio
async def test_head_response_suppresses_body_and_preserves_content_length() -> None:
    async def app(scope, receive, send):
        await receive()
...
@pytest.mark.asyncio
async def test_pipelined_requests_and_204_no_body_do_not_desynchronize_connection() -> None:
    async def app(scope, receive, send):
        await receive()
        if scope["path"] == "/empty":
    ...
    try:
        ...
        assert b"204 No Content" in head1
        assert b"should-not-be-sent" not in head1
        assert body1 == b""
        head2, body2 = await _read_response(reader)
        assert b"200 OK" in head2
        assert body2 == b"/after"
        writer.close()
        await writer.wait_closed()
    finally:
        await server.close()
