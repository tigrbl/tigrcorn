...
def test_actual_repository_release_gates_pass_for_the_committed_tree():
    report = evaluate_release_gates(ROOT)
    assert report.passed, '\n'.join(report.failures)
    assert report.failures == []
    assert report.rfc_status['RFC 9114']['highest_observed_evidence_tier'] == 'independent_certification'
    assert report.rfc_status['RFC 9220']['highest_observed_evidence_tier'] == 'independent_certification'
    assert report.rfc_status['RFC 9002']['highest_observed_evidence_tier'] == 'independent_certification'
def test_synthetic_release_tree_passes_when_boundary_evidence_and_artifacts_align():
...
def test_actual_repository_promotion_evaluator_keeps_performance_and_docs_green():
