...
class QuicRFCUpgradePathTests(unittest.TestCase):
    def test_transport_parameters_binary_roundtrip_preserves_extensions(self):
        ...
        self.assertEqual(decoded.max_data, 123456)
        self.assertEqual(decoded.max_stream_data_uni, 4444)
        self.assertEqual(decoded.active_connection_id_limit, 6)
        self.assertTrue(decoded.disable_active_migration)
    ...
    def test_server_handshake_flight_is_split_across_initial_and_handshake_spaces(self):
        ...
        self.assertEqual([flight.packet_space for flight in flights], ['initial', 'handshake'])
    ...
    def test_crypto_reassembly_waits_for_missing_prefix(self):
        ...
        self.assertEqual(server.take_handshake_datagrams(), [])
        server.receive_datagram(client.send_crypto_data(payload[:midpoint], offset=0, packet_space='initial'))
        self.assertTrue(server.take_handshake_datagrams())
    ...
    def test_key_update_allows_post_update_short_packets(self):
        ...
        self.assertTrue(any(event.kind == 'stream' and event.data == b'before' for event in events))
        left.initiate_key_update()
        events = right.receive_datagram(left.send_stream_data(4, b'after', fin=True))
        self.assertTrue(any(event.kind == 'stream' and event.data == b'after' for event in events))


class QuicRecoveryUpgradePathTests(unittest.TestCase):
    def test_packet_number_spaces_are_isolated(self):
        ...
        self.assertIn(1, recovery.spaces['initial'].outstanding)
        self.assertNotIn(1, recovery.spaces['application'].outstanding)
    ...
    def test_persistent_congestion_collapses_congestion_window(self):
        ...
        self.assertTrue(recovery.persistent_congestion)
        self.assertEqual(recovery.congestion_window, recovery.minimum_congestion_window)
    ...
    def test_pacing_budget_replenishes_after_time_passes(self):
        ...
        self.assertLess(depleted_budget, recovery.congestion_window)
        self.assertGreater(replenished_budget, depleted_budget)
