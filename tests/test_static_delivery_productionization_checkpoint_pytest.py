...
async def _read_h3_response_with_client_progress(
    ...
    for _ in range(128):
        ...
        for raw in (
            client.take_handshake_datagrams() + client.drain_scheduled_datagrams()
        ):
...
@pytest.mark.asyncio
async def test_static_files_app_uses_streaming_file_extension_when_available():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with patch(
            ...
            side_effect=AssertionError(
                "read_bytes must not be used for streaming static delivery"
            ),
        ):
            await app(scope, receive, send)
        assert sent[0]["type"] == "http.response.start"
        assert sent[0]["status"] == 200
        assert sent[1]["type"] == "tigrcorn.http.response.file"
        assert sent[1]["segments"][0]["type"] == "file"
        assert Path(sent[1]["segments"][0]["path"]).resolve(strict=False) == (
            root / "blob.bin"
        ).resolve(strict=False)
        assert sent[1]["segments"][0]["offset"] == 0
        assert sent[1]["segments"][0]["count"] == len(payload)


@pytest.mark.asyncio
async def test_http11_large_static_file_serves_without_read_bytes():
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        ...
        try:
            with patch(
                "pathlib.Path.read_bytes",
                side_effect=AssertionError(
                    "read_bytes must not be used for HTTP/1.1 static delivery"
                ),
            ):
...
@pytest.mark.asyncio
async def test_http2_large_static_range_serves_without_read_bytes():
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        ...
        try:
            with patch(
                "pathlib.Path.read_bytes",
                side_effect=AssertionError(
                    "read_bytes must not be used for HTTP/2 static delivery"
                ),
            ):
                reader, writer = await asyncio.open_connection("127.0.0.1", port)
                ...
                assert header_map[b":status"] == b"206"
                assert header_map[b"content-range"] == f"bytes 4096-8191/{len(payload)}".encode(
                    "ascii"
                )
                assert body == payload[4096:8192]
                writer.close()
                await writer.wait_closed()
        finally:
            await server.close()
...
@pytest.mark.asyncio
async def test_http3_large_static_range_serves_without_read_bytes():
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        ...
        try:
            with patch(
                "pathlib.Path.read_bytes",
                side_effect=AssertionError(
                    "read_bytes must not be used for HTTP/3 static delivery"
                ),
            ):
                ...
                header_map = dict(response_headers)
                assert header_map[b":status"] == b"206"
                assert header_map[b"content-range"] == f"bytes 16384-32767/{len(payload)}".encode(
                    "ascii"
                )
                assert body == payload[16384:32768]
        finally:
            sock.close()
            await server.close()
