...
from tigrcorn.protocols.http3.codec import FRAME_HEADERS as H3_FRAME_HEADERS, parse_frames
...
class ResponseTrailerFieldsRFC9110Tests(unittest.IsolatedAsyncioTestCase):
    async def test_http11_response_trailers_are_emitted(self):
        ...
        try:
            ...
            rest = await asyncio.wait_for(reader.readuntil(b'0\r\nx-trailer-one: yes\r\nx-trailer-two: done\r\n\r\n'), 2.0)
            response = head + rest
            self.assertIn(b'transfer-encoding: chunked', response.lower())
            self.assertIn(b'0\r\nx-trailer-one: yes\r\nx-trailer-two: done\r\n\r\n', response.lower())
            writer.close(); await writer.wait_closed()
        finally:
            await server.close()

    async def test_http2_response_trailers_are_emitted(self):
        ...
        try:
            ...
            buf = FrameBuffer()
            response_headers=[]; response_trailers=[]; body=bytearray(); ended=False
            while not ended:
            ...
            self.assertIn((b'x-trailer-one', b'yes'), response_trailers)
            self.assertIn((b'x-trailer-two', b'done'), response_trailers)
            writer.close(); await writer.wait_closed()
        finally:
            await server.close()

    async def test_http3_response_trailers_are_emitted(self):
        ...
        try:
            ...
            self.assertIn((b'x-trailer-one', b'yes'), response_state.trailers)
            self.assertIn((b'x-trailer-two', b'done'), response_state.trailers)
        finally:
            sock.close()
            await server.close()
