...
def test_sanitize_103_allows_only_link_and_strips_connection_specific_headers():
    ...
    assert headers == [(b'link', b'</app.js>; rel=preload; as=script')]


@pytest.mark.asyncio
async def test_http11_103_is_emitted_before_final_response():
...
@pytest.mark.asyncio
async def test_http2_103_is_emitted_before_final_headers():
    async def app(scope, receive, send):
        await receive()
    ...
    try:
        ...
        assert (b':status', b'103') in header_blocks[0]
        assert (b'link', b'</app.css>; rel=preload; as=style') in header_blocks[0]
        assert (b'x-unsafe', b'no') not in header_blocks[0]
        assert (b':status', b'200') in header_blocks[-1]
        assert body == b'ok'
        writer.close()
        await writer.wait_closed()
    finally:
...
@pytest.mark.asyncio
async def test_http3_103_is_emitted_before_final_headers():
    async def app(scope, receive, send):
        await receive()
        await send({'type': 'http.response.start', 'status': 103, 'headers': [(b'link', b'</app.js>; rel=preload; as=script'), (b'x-unsafe', b'no')]})
        await send({'type': 'http.response.start', 'status': 200, 'headers': [(b'content-type', b'text/plain')]})
