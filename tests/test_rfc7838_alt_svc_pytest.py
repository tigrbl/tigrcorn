...
def test_explicit_auto_and_suppression_behavior():
    ...
    assert configured_alt_svc_values(explicit, request_http_version='3') == [b'h3=":9443"; ma=60']


def test_append_alt_svc_headers_preserves_existing_field():
    ...
    assert headers == [(b'alt-svc', b'h3=":1111"; ma=60')]


@pytest.mark.asyncio
async def test_http11_auto_advertises_alt_svc_header_field():
    ...
    try:
        ...
        assert body == b'ok'
        assert headers[b'alt-svc'] == f'h3=":{udp_port}"; ma=86400'.encode('ascii')
        writer.close()
...
@pytest.mark.asyncio
async def test_http2_auto_advertises_alt_svc_header_field():
    async def app(scope, receive, send):
        await receive()
        await send({'type': 'http.response.start', 'status': 200, 'headers': [(b'content-type', b'text/plain')]})
        await send({'type': 'http.response.body', 'body': b'ok', 'more_body': False})
    ...
    try:
        ...
        assert (b'alt-svc', f'h3=":{udp_port}"; ma=86400'.encode('ascii')) in headers
        assert body == b'ok'
        writer.close()
        await writer.wait_closed()
    finally:
...
@pytest.mark.asyncio
async def test_http3_can_emit_explicit_alt_svc_header_field():
    async def app(scope, receive, send):
        await receive()
        await send({'type': 'http.response.start', 'status': 200, 'headers': [(b'content-type', b'text/plain')]})
        await send({'type': 'http.response.body', 'body': b'ok', 'more_body': False})
    ...
    try:
        ...
        assert (b'alt-svc', b'h3=":9443"; ma=60') in response_state.headers
