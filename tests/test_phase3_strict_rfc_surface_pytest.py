...
async def test_cli_phase3_flags_round_trip_into_config():
    parser = build_parser()
    ...
    assert config.tls.ocsp_mode == 'require'
    assert config.tls.ocsp_soft_fail
    assert config.tls.ocsp_cache_size == 64
    assert config.tls.ocsp_max_age == 30
    assert config.tls.crl_mode == 'soft-fail'
    assert not (config.tls.revocation_fetch)
    assert config.tls.alpn_protocols == ['h2', 'http/1.1']
    assert config.http.connect_policy == 'allowlist'
    assert config.http.connect_allow == ['127.0.0.1:443', '10.0.0.0/8']
    assert config.http.trailer_policy == 'drop'
    assert config.http.content_coding_policy == 'strict'
    assert config.http.content_codings == ['gzip', 'deflate']
    assert config.websocket.compression == 'permessage-deflate'
async def test_http11_websocket_compression_auto_negotiates_when_enabled():
    seen = {}
...
async def test_http11_websocket_compression_policy_off_strips_app_requested_extension():
    async def app(scope, receive, send):
        await receive()
    ...
    try:
        ...
        await writer.drain()
        response = await reader.readuntil(b'\r\n\r\n')
        assert b'sec-websocket-extensions:' not in response.lower()
        writer.close()
        await writer.wait_closed()
    finally:
        await server.close()

async def test_http11_connect_policy_deny_and_allowlist():
    received = bytearray()
...
async def test_http11_trailer_policy_drop_suppresses_trailer_event():
    seen_types = []
    ...
    try:
        ...
        assert 'http.request.trailers' not in seen_types
        writer.close()
        await writer.wait_closed()
    finally:
        await server.close()

async def test_http11_content_coding_identity_only_disables_gzip():
    async def app(scope, receive, send):
        await receive()
    ...
    try:
        ...
        await writer.drain()
        head = await reader.readuntil(b'\r\n\r\n')
        assert b'content-encoding: gzip' not in head.lower()
        writer.close()
        await writer.wait_closed()
    finally:
        await server.close()

def test_build_server_ssl_context_uses_public_alpn_and_revocation_policy():
    ...
    assert context.validation_policy.revocation_mode.value == 'require'
    assert context.validation_policy.revocation_fetch_policy is None
