...
def test_corpus_matches_expected_vector_catalog() -> None:
    vectors = _load_corpus()
    assert len(vectors) == len(EXPECTED_VECTOR_CATALOG)
    seen_vector_ids: set[str] = set()
    for vector in vectors:
        assert vector.rfc not in seen_vector_ids, vector.rfc
        seen_vector_ids.add(vector.rfc)
        assert vector.rfc in EXPECTED_VECTOR_CATALOG
        expected = EXPECTED_VECTOR_CATALOG[vector.rfc]
        assert vector.name == expected["name"]
        assert vector.protocol == expected["protocol"]
        assert vector.fixture == expected["fixture"]
        assert len(vector.description.split()) >= 8, vector.rfc
    assert seen_vector_ids == set(EXPECTED_VECTOR_CATALOG)


def test_documented_review_surface_is_a_subset_of_the_corpus() -> None:
    vectors = _load_corpus()
    ...
    assert not (required_rfcs - corpus_rfcs), sorted(required_rfcs - corpus_rfcs)


def test_vector_names_and_protocol_rfc_pairs_are_unique() -> None:
    vectors = _load_corpus()
    assert len({vector.name for vector in vectors}) == len(vectors)
    assert len({(vector.protocol, vector.rfc) for vector in vectors}) == len(vectors)


def test_every_vector_points_to_a_real_test_fixture() -> None:
    vectors = _load_corpus()
    for vector in vectors:
        fixture = ROOT / vector.fixture
        assert fixture.exists(), vector.fixture
        contents = fixture.read_text()
        assert ("def test_" in contents or "class " in contents), vector.fixture
