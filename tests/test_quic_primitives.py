...
class QuicPrimitiveTests(unittest.TestCase):
    def test_secret_and_payload_protection(self):
        ...
        self.assertEqual(unprotect_payload(key, 3, protected), payload)
    ...
    def test_datagram_roundtrip(self):
        ...
        self.assertEqual(decoded.header.packet_number, 5)
        self.assertEqual(decoded.payload, b'data')
        self.assertEqual(decoded.tag, b'tag')

    def test_frame_roundtrip(self):
        frame = QuicStreamFrame(stream_id=0, data=b'abc', fin=True)
        decoded, _ = decode_frame(encode_frame(frame))
        self.assertEqual(decoded.stream_id, 0)
    ...
    def test_ack_frame_with_ranges_roundtrip(self):
        ...
        decoded, _ = decode_frame(encode_frame(frame))
        self.assertEqual(decoded.largest_acked, 10)
        self.assertEqual(decoded.ack_delay, 3)
        self.assertEqual(decoded.first_ack_range, 2)
        self.assertEqual(decoded.ack_ranges, [(1, 1), (0, 0)])

    def test_flow_control(self):
        flow = QuicFlowControl(connection_window=10)
        self.assertTrue(flow.can_send(0, 5))
        flow.consume_send(0, 5)
        self.assertFalse(flow.can_send(0, 6))
        flow.credit_connection(10)
        flow.credit_stream(0, 10)
        self.assertTrue(flow.can_send(0, 6))
    ...
    def test_quic_connection_ack(self):
        ...
        self.assertTrue(any(event.kind == 'ping' for event in events))
        ack = server.acknowledge(0)
        client_events = client.receive_datagram(ack)
        self.assertTrue(any(event.kind == 'ack' for event in client_events))
