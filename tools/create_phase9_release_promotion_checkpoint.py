...
VERSION = '0.3.8'
RELEASE_ROOT_TEXT = 'docs/review/conformance/releases/0.3.8/release-0.3.8'
...
RELEASE_NOTES = 'RELEASE_NOTES_0.3.8.md'
...
def update_external_matrices() -> None:
    ...
    metadata['description'] = 'Canonical independent certification matrix for the current 0.3.8 release bundle. Every enabled scenario has preserved passing artifacts in the canonical independent bundle.'
    ...
    same_stack_payload['metadata']['description'] = 'Replayable same-stack fixture matrix for HTTP/3, QUIC-TLS feature axes, and RFC 9220 coverage. These scenarios are useful regression evidence and are preserved under the canonical 0.3.8 release root, but they are not independent certification artifacts.'
    ...
    current_release_payload['metadata']['description'] = 'Mixed evidence matrix that combines independent HTTP/1.1 / HTTP/2 / WebSocket peers with same-stack replay fixtures for HTTP/3, QUIC-TLS, and RFC 9220 coverage under the canonical 0.3.8 release root.'
...
def update_root_bundle_metadata() -> None:
    ...
    release_manifest['notes'] = [
        ...
        'The 0.3.8 release root is now the canonical authoritative release root and the public package version is 0.3.8.',
    ]
    ...
    for rel in [
        ...
        payload['commit_hash'] = 'release-0.3.8'
        env = payload.setdefault('environment', {})
        env['curation'] = {
            'note': 'This canonical 0.3.8 release root consolidates preserved evidence from the historical 0.3.2, 0.3.6-current, 0.3.6-rfc-hardening, and Phase 9 closure artifacts while keeping the underlying packet traces and peer transcripts intact.'
        }
        env['tigrcorn'] = {
            'bundle_scope': 'curated-preserved-artifacts',
            'commit_hash': 'release-0.3.8',
            'version': VERSION,
        }
    ...
    indep_summary['commit_hash'] = 'release-0.3.8'
...
def update_markdown_docs() -> None:
    ...
    for doc in [ROOT / 'docs/protocols/http3.md', ROOT / 'docs/protocols/quic.md', ROOT / 'docs/protocols/websocket.md']:
        ...
        text = text.replace('canonical `0.3.6` same-stack bundle', 'canonical `0.3.8` same-stack bundle')
...
def create_release_notes_and_promotion_docs(authoritative_passed: bool, strict_passed: bool, promotion_passed: bool) -> None:
    release_notes_text = f"""# Release notes — tigrcorn 0.3.8\n\n`tigrcorn` `0.3.8` promotes the previously validated 0.3.8 release root to the canonical authoritative release.\n\n## What changed in this promotion\n\n- promoted `docs/review/conformance/releases/0.3.8/release-0.3.8/` to the canonical release root\n- updated `pyproject.toml` from `0.3.6` to `0.3.8`\n- aligned the authoritative release-boundary metadata, external matrices, release manifests, summaries, and current-state snapshots with the promoted 0.3.8 release\n- preserved the Step 8 strict-validation bundle and its passing results\n\n## Current certification status\n\n- authoritative boundary: `{authoritative_passed}`\n- strict target boundary: `{strict_passed}`\n- composite promotion target: `{promotion_passed}`\n\nThe promoted 0.3.8 release is **certifiably fully RFC compliant**, **strict-target certifiably fully RFC compliant**, and **certifiably fully featured**.\n\n## Notable preserved bundles\n\n- independent certification matrix\n- same-stack replay matrix\n- mixed compatibility matrix\n- flag-surface certification bundle\n- operator-surface certification bundle\n- performance certification bundle\n- certification-environment freeze bundle\n- aioquic adapter preflight bundle\n- strict-validation bundle\n\n## Operational note\n\nThe local workspace used for this checkpoint still runs under Python 3.13. The frozen release-workflow contract remains Python 3.11 or 3.12 with `.[certification,dev]` installed. That does not change the preserved release-artifact truth of the canonical 0.3.8 release root.\n"""
...
def update_tests() -> None:
    ...
    text = text.replace("self.assertEqual(manifest_payload['environment']['tigrcorn']['commit_hash'], 'release-0.3.6')", "self.assertEqual(manifest_payload['environment']['tigrcorn']['commit_hash'], 'release-0.3.8')")
    text = text.replace("self.assertEqual(manifest_payload['environment']['tigrcorn']['version'], '0.3.6')", "self.assertEqual(manifest_payload['environment']['tigrcorn']['version'], '0.3.8')")
    path.write_text(text, encoding='utf-8')
...
EXPECTED_PENDING_SCENARIO_IDS: set[str] = set()"""
    ...
    text = path.read_text(encoding='utf-8')
    text = text.replace("assert status['current_state']['current_package_version'] == '0.3.6'", "assert status['current_state']['current_package_version'] == '0.3.8'")
