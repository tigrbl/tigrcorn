...
STATUS_JSON = CONFORMANCE / 'connect_relay_independent_connect_relay_independent.current.json'
STATUS_MD = CONFORMANCE / 'CONNECT_RELAY_INDEPENDENT_CONNECT_RELAY_INDEPENDENT_CLOSURE.md'
DELIVERY_NOTES = ROOT / 'docs/review/conformance/delivery/DELIVERY_NOTES_CONNECT_RELAY_INDEPENDENT_CONNECT_RELAY_INDEPENDENT_CLOSURE.md'
TMP_ROOT = ROOT / '.artifacts' / 'connect_relay_independent_connect_relay_runs'
SCENARIOS = [
    'http11-connect-relay-curl-client',
    'http2-connect-relay-h2-client',
    'http3-connect-relay-aioquic-client',
]
COMMIT_HASH = 'connect_relay_independent-connect-relay-checkpoint'
...
def _overlay_generated_scenarios(generated_root: Path) -> None:
    ...
    note = 'CONNECT relay independent closure overlays fresh CONNECT relay independent-artifact runs for HTTP/1.1, HTTP/2, and HTTP/3 into the 0.3.9 working release root.'
...
def _create_local_negative_bundle() -> None:
    ...
    vectors = [
        ...
        ('http2-connect-policy-deny', True, ['tests/test_connect_relay_local_negatives.py::ConnectRelayIndependentLocalNegativeTests::test_http2_connect_policy_deny_and_allowlist_rejection_end_stream']),
        ('http2-connect-allowlist-rejection', True, ['tests/test_connect_relay_local_negatives.py::ConnectRelayIndependentLocalNegativeTests::test_http2_connect_policy_deny_and_allowlist_rejection_end_stream']),
        ('http3-connect-policy-deny', True, ['tests/test_connect_relay_local_negatives.py::ConnectRelayIndependentLocalNegativeTests::test_http3_connect_policy_deny_and_allowlist_rejection_end_stream']),
        ('http3-connect-allowlist-rejection', True, ['tests/test_connect_relay_local_negatives.py::ConnectRelayIndependentLocalNegativeTests::test_http3_connect_policy_deny_and_allowlist_rejection_end_stream']),
    ]
    scenarios = []
    ...
    _write_json(
        LOCAL_NEGATIVE_ROOT / 'manifest.json',
        {
            ...
            'generated_at': _now(),
            'commit_hash': COMMIT_HASH,
            'description': 'Local CONNECT policy-negative vectors preserved during CONNECT relay independent closure independent CONNECT relay closure work.',
        },
    )
    _write_json(
...
def _update_release_root_manifest() -> None:
    ...
    payload.update(
        {
            'status': 'connect_relay_independent_connect_relay_complete_all_carriers_green',
            'promotion_ready': False,
            'strict_target_complete': False,
            'source_checkpoint': 'connect_relay_independent_connect_relay_independent_closure',
            'generated_at': _now(),
            'bundles': bundles,
        }
    )
    notes = list(payload.get('notes', []))
    note = 'CONNECT relay independent closure overlays RFC 9110 CONNECT relay independent artifacts for HTTP/1.1, HTTP/2, and HTTP/3 into the 0.3.9 working release root. The remaining strict-target blockers are now trailer-fields and content-coding only.'
    if note not in notes:
        notes.append(note)
    payload['notes'] = notes
