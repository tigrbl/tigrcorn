...
DEFAULT_CHECKPOINT_SCRIPTS = (
    'tools/create_independent_harness_foundation_checkpoint.py',
    'tools/create_rfc7692_independent_closure_checkpoint.py',
    'tools/create_connect_relay_independent_closure_checkpoint.py',
    'tools/create_trailer_fields_independent_closure_checkpoint.py',
    'tools/create_content_coding_independent_closure_checkpoint.py',
    'tools/create_ocsp_certification_checkpoint.py',
    'tools/create_strict_performance_checkpoint.py',
    'tools/create_release_assembly_checkpoint.py',
)


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description='Run the Release certification release workflow only after freezing and validating the certification environment.'
    )
    parser.add_argument(
        '--scripts',
        nargs='*',
        default=list(DEFAULT_CHECKPOINT_SCRIPTS),
        help='Repository-relative Release certification checkpoint scripts to execute after the certification environment has been frozen.',
    )
    parser.add_argument(
        '--release-root',
...
def main() -> int:
    ...
    freeze_command = [
        ...
        '--wrapper-path',
        'tools/run_certification_release_workflow.py',
    ]
