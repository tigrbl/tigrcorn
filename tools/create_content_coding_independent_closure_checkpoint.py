...
TMP_ROOT = ROOT / '.artifacts' / 'content_coding_independent_content_coding_runs'
SCENARIOS = [
    'http11-content-coding-curl-client',
    'http2-content-coding-curl-client',
    'http3-content-coding-aioquic-client',
]
COMMIT_HASH = 'content_coding_independent-content-coding-checkpoint'
...
def _overlay_generated_scenarios(generated_root: Path) -> None:
    ...
    note = 'Content coding independent closure overlays fresh content-coding independent-artifact runs for HTTP/1.1, HTTP/2, and HTTP/3 into the 0.3.9 working release root.'
...
def _create_local_behavior_bundle() -> None:
    ...
    _write_json(
        LOCAL_BEHAVIOR_ROOT / 'manifest.json',
        {
            ...
            'generated_at': _now(),
            'commit_hash': COMMIT_HASH,
            'description': 'Local content-coding behavior vectors preserved during Content coding independent closure independent content-coding closure work.',
        },
    )
    _write_json(
...
def _update_release_root_manifest() -> None:
    ...
    payload.update(
        {
            'status': 'content_coding_independent_content_coding_complete_all_carriers_green' if content_coding_green else 'content_coding_independent_content_coding_partial_http11_http2_green_http3_blocked',
            'promotion_ready': content_coding_green,
            'strict_target_complete': content_coding_green,
            'source_checkpoint': 'content_coding_independent_content_coding_independent_closure',
            'generated_at': _now(),
            'bundles': bundles,
        }
    )
    notes = list(payload.get('notes', []))
    green_note = 'Content coding independent closure preserves RFC 9110 content-coding independent artifacts as passing third-party evidence across HTTP/1.1, HTTP/2, and HTTP/3 under the 0.3.9 working release root.'
    blocked_note = 'Content coding independent closure overlays RFC 9110 content-coding independent artifacts for HTTP/1.1 and HTTP/2, plus a preserved HTTP/3 failure artifact caused by the missing aioquic runtime dependency in the current environment.'
    ...
    marker = 'Content coding independent closure content-coding status:'
    line = ('Content coding independent closure content-coding status: HTTP/1.1, HTTP/2, and HTTP/3 preserved independent artifacts are now all passing, so RFC 9110 §8 no longer blocks the strict target.' if content_coding_green else 'Content coding independent closure content-coding status: HTTP/1.1 and HTTP/2 independent artifacts are passing, while HTTP/3 remains a preserved failing artifact in this environment.')
    if marker in text:
