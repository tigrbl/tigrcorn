...
STATUS_JSON = CONFORMANCE / 'trailer_fields_independent_trailer_fields_independent.current.json'
STATUS_MD = CONFORMANCE / 'TRAILER_FIELDS_INDEPENDENT_TRAILER_FIELDS_INDEPENDENT_CLOSURE.md'
...
DELIVERY_NOTES = ROOT / 'docs/review/conformance/delivery/DELIVERY_NOTES_TRAILER_FIELDS_INDEPENDENT_TRAILER_FIELDS_INDEPENDENT_CLOSURE.md'
TMP_ROOT = ROOT / '.artifacts' / 'trailer_fields_independent_trailer_fields_runs'
SCENARIOS = [
    'http11-trailer-fields-curl-client',
    'http2-trailer-fields-h2-client',
    'http3-trailer-fields-aioquic-client',
]
COMMIT_HASH = 'trailer_fields_independent-trailer-fields-checkpoint'
...
def _overlay_generated_scenarios(generated_root: Path) -> None:
    ...
    note = 'Trailer fields independent closure overlays fresh trailer-field independent-artifact runs for HTTP/1.1, HTTP/2, and HTTP/3 into the 0.3.9 working release root.'
...
def _create_local_behavior_bundle() -> None:
    ...
    _write_json(
        LOCAL_BEHAVIOR_ROOT / 'manifest.json',
        {
            ...
            'generated_at': _now(),
            'commit_hash': COMMIT_HASH,
            'description': 'Local request/response trailer behavior vectors preserved during Trailer fields independent closure independent trailer-field closure work.',
        },
    )
    _write_json(
...
def _update_release_root_manifest() -> None:
    ...
    payload.update(
        {
            'status': 'trailer_fields_independent_trailer_fields_complete_all_carriers_green',
            'promotion_ready': False,
            'strict_target_complete': False,
            'source_checkpoint': 'trailer_fields_independent_trailer_fields_independent_closure',
            'generated_at': _now(),
            'bundles': bundles,
        }
    )
    notes = list(payload.get('notes', []))
    note = 'Trailer fields independent closure now preserves passing RFC 9110 trailer-field independent artifacts for HTTP/1.1, HTTP/2, and HTTP/3 under the 0.3.9 working release root. The remaining strict-target blocker is now the HTTP/3 content-coding scenario only.'
    if note not in notes:
        notes.append(note)
    payload['notes'] = notes
    ...
    if 'Trailer fields independent closure adds trailer-field independent-artifact overlays' not in text:
        text += '\nTrailer fields independent closure adds trailer-field independent-artifact overlays for HTTP/1.1 and HTTP/2, plus a preserved HTTP/3 failing artifact in this environment.\n'
